...
- 必须兼容 Qt 5.13.2（Windows install 版最低目标）。新增或修改 Qt API 调用时，先确认 Qt 5.13.2 是否支持；若只在更高版本可用，必须用 `#if QT_VERSION >= QT_VERSION_CHECK(...)` 提供 5.13.2 分支。例如 `QWheelEvent::position()` 需在 Qt 5.13.2 下回退到 `pos()`。
